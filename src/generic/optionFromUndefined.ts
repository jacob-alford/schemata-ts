...
export type SchemableParams2<S extends URIS2> = <A, E>(
  sa: Kind2<S, E, A>,
) => Kind2<S, E | undefined, O.Option<A>>
...
export const Decoder: SchemableParams2C<D.URI> = decoderA => ({
  decode: u =>
    u === undefined ? D.success(O.none) : pipe(decoderA.decode(u), E.map(O.some)),
})
...
export const Encoder: SchemableParams2<Enc.URI> = encoderA => ({
  encode: flow(O.map(encoderA.encode), O.toUndefined),
})
...
export const TaskDecoder: SchemableParams2C<TD.URI> = taskDecoderA => ({
  decode: u =>
    u === undefined ? TD.success(O.none) : pipe(taskDecoderA.decode(u), TE.map(O.some)),
})
